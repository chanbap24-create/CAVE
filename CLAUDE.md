...
## Development Rules
1. **코드는 최대한 단순하게**: 추상화·옵션·플래그·헬퍼를 미리 도입하지 말 것. 지금 이 작업이 요구하는 만큼만 구현. 두 줄로 충분하면 한 줄짜리 헬퍼 만들지 말고, 같은 코드가 세 번째 등장할 때 그제야 추출.
2. 많은 부분을 수정해야 한다면 반드시 사용자에게 물어보고 진행할 것
3. 하나의 파일에 코드를 다 넣지 말고, 기능별로 모듈화 할 것
4. 요청이 명확하지 않을 때 추론 및 실행하지 말고 우선 사용자의 설명을 제대로 이해했는지 확인할 것
5. **중복 즉시 추출**: 같은 헬퍼/상수/env 로드가 2개 이상 파일에 나타나면 곧바로 공용 모듈로 추출할 것
   - Edge Functions 공용: `supabase/functions/_shared/`
   - 모바일 공용: `apps/mobile/lib/` (훅은 `lib/hooks/`, 유틸은 `lib/utils/`)
6. **파일 크기 상한**: 한 화면/컴포넌트 파일은 약 200줄 이하 유지. 초과 시 hooks/subcomponents로 분할
7. **매직 넘버 금지**: TTL, rate limit, threshold 등은 `_shared` 또는 `lib/config` 상수로
