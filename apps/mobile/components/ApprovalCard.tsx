...
/**
 * Single pending-approval card. Handles both wine_change (old→new swap
 * display) and no_wine_apply (short explanation) request types behind the
 * same voting UI.
 */
export function ApprovalCard({
  ...
  const isNoWineApply = approval.request_type === 'no_wine_apply';
  const headerText = isNoWineApply
    ? `@${approval.requester_username ?? '—'} 님이 빈손으로 참가 요청`
    : `@${approval.requester_username ?? '—'} 님이 변경 요청`;

  return (
    <View style={styles.card}>
      <Text style={styles.requester}>{headerText}</Text>

      {isNoWineApply ? (
        <Text style={styles.noWineBody}>
          와인 없이 참가를 신청했습니다. 참가 승인에 전원 동의가 필요합니다.
        </Text>
      ) : (
      ...
      {approval.note ? <Text style={styles.note}>"{approval.note}"</Text> : null}
      ...
      {isRequester ? (
        <Pressable style={[styles.actionBtn, styles.cancelBtn]} onPress={() => onCancel(approval.id)}>
        ...
        myVote ? (
          ...
          <View style={styles.voteRow}>
            <Pressable style={[styles.actionBtn, styles.approveBtn]} onPress={() => onVote(approval.id, 'approve')}>
              <Text style={styles.approveText}>승인</Text>
            </Pressable>
            <Pressable style={[styles.actionBtn, styles.rejectBtn]} onPress={() => onVote(approval.id, 'reject')}>
...
function WineMini({ wine, label, dimmed }: { wine: ApprovalWine | null; label: string; dimmed?: boolean }) {
  return (
    <View style={[styles.wineMini, dimmed && { opacity: 0.55 }]}>
      ...
      <Text style={styles.miniName} numberOfLines={2}>{wine?.name ?? '—'}</Text>
      {wine?.producer ? (
...
const styles = StyleSheet.create({
  ...
  miniThumb: { width: 56, height: 56, borderRadius: 8, backgroundColor: '#f0f0f0', marginBottom: 6 },
  miniPlaceholder: { backgroundColor: '#f0f0f0' },
  miniLabel: { fontSize: 10, fontWeight: '600', color: '#8a6d3b', marginBottom: 2 },
  miniName: { fontSize: 12, fontWeight: '600', color: '#222', textAlign: 'center', lineHeight: 16 },
  miniProducer: { fontSize: 10, color: '#7b2d4e', fontWeight: '500', marginTop: 2, textAlign: 'center' },

  note: {
    fontSize: 12, color: '#555', marginTop: 12,
    backgroundColor: '#fff', padding: 10, borderRadius: 8, lineHeight: 17,
  },
  noWineBody: { fontSize: 12, color: '#8a6d3b', marginTop: 2, lineHeight: 17 },

  tally: { marginTop: 12 },
  tallyText: { fontSize: 12, fontWeight: '600', color: '#555', marginBottom: 6 },
  progressTrack: { height: 6, backgroundColor: '#f0e2c8', borderRadius: 3, overflow: 'hidden' },
  ...
  cancelBtn: { marginTop: 14, backgroundColor: '#fff', borderWidth: 1, borderColor: '#ed4956' },
  cancelText: { color: '#ed4956', fontSize: 13, fontWeight: '600' },

  votedBadge: { marginTop: 14, paddingVertical: 10, backgroundColor: '#f0f0f0', borderRadius: 8, alignItems: 'center' },
  votedText: { fontSize: 12, fontWeight: '600', color: '#555' },

  waiting: { marginTop: 14, textAlign: 'center', fontSize: 11, color: '#999', fontStyle: 'italic' },
});
